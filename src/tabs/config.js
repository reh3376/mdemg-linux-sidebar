...
export function renderConfig(container) {
  ...
  // Instance Removal
  frag.appendChild(sectionHeader('Instance Removal'));
  frag.appendChild(h('div', { className: 'info-value', style: { fontSize: '11px', padding: '2px 0 6px', textAlign: 'left', maxWidth: '100%' } },
    'Completely remove this MDEMG instance, including server, database, hooks, and configuration.'));
  const removeBtn = button('Remove Instance...', () => {
    // Dynamic import to avoid circular dependency
    import('./teardown-wizard.js').then(m => m.openTeardownWizard());
  }, { destructive: true, disabled: s.isTeardownRunning });
  frag.appendChild(h('div', { className: 'button-row' }, removeBtn));
